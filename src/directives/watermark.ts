export default {
  created() {},
  beforeMount() {},
  mounted(el: HTMLElement, { value }: { value: Record<string, any> | string }) {
    ...
    el['$setMark'] = (value: Record<string, any> | string) => {
      ...
      el['style'].backgroundImage = `url(${el['_canvas']['toDataURL'](
        'image/png'
      )})`
    }
    el['$setMark'](value)
  ...
  beforeUpdate() {},
  updated(el: HTMLElement, { value }: { value: Record<string, any> | string }) {
    el['$setMark'](value)
  },
  beforeUnmount() {},
  unMounted() {}
